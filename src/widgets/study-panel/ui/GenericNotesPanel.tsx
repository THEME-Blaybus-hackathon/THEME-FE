...
import {
  ...
  useUpdateMemo,
  useDeleteMemo,
} from '@/entities/note/api/queries';
                ...
                <ActionGroup onClick={(e) => e.stopPropagation()}>
                  {isExpanded &&
                    (isEditing ? (
                      ...
                      <IconButton onClick={() => startEdit(memo)}>✏️</IconButton>
                    ))}
                  <IconButton
                    onClick={() =>
...
const MemoItem = styled.div<{ isExpanded: boolean }>`
  background: ${(props) =>
    props.isExpanded ? 'rgba(255, 255, 255, 0.12)' : 'rgba(255, 255, 255, 0.06)'};
  border-radius: 15px;
...
const ItemHeader = styled.div`
  ...
  .date { color: rgba(255, 255, 255, 0.3); font-size: 11px; }
`;
const ItemBody = styled.div`
  padding: 0 15px 15px 15px;
  p { color: rgba(255, 255, 255, 0.6); font-size: 12px; line-height: 1.6; margin-top: 10px; white-space: pre-wrap; }
`;
