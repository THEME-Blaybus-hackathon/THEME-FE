...
const PART_TITLE_MATCHERS: {
  ...
  { match: (name) => name.includes('Spring'), title: 'Coil Spring' },
  { match: (name) => name.includes('Rod'), title: 'Shock Rod' },
  {
    match: (name) => name.includes('Nut') || name.includes('Nit'),
    title: 'Lock Nut',
  },
  { match: (name) => name.includes('Base'), title: 'Suspension Base' },
  { match: (name) => name.includes('Solid1'), title: 'Fixing Pin' },
];
