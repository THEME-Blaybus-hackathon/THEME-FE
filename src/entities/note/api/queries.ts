...
import { getMemosByPart, createMemo, updateMemo, deleteMemo } from './noteApi';
...
export function useUpdateMemo() {
  ...
  return useMutation({
    mutationFn: (vars: {
      ...
      title: string;
      partName: string;
    }) => updateMemo(vars.id, vars.content, vars.title, vars.partName),
    onSuccess: (_, vars) => {
      queryClient.invalidateQueries({ queryKey: ['memos', vars.partName] });
      alert('수정되었습니다!');
    },
...
export function useDeleteMemo() {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: (vars: { id: number; partName: string }) =>
      deleteMemo(vars.id),
    onSuccess: (_, vars) => {
      queryClient.invalidateQueries({ queryKey: ['memos', vars.partName] });
      alert('삭제되었습니다!');
